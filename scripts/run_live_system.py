...
def main():
    ...
    if not run_step("python scripts/run_pipeline.py", "Priming Base Data Pipeline"):
    ...
    try:
        ...
        print("▶️  Starting Background Batch Daemon (scripts/pipeline_daemon.py)")
        daemon_proc = subprocess.Popen([sys.executable, "scripts/pipeline_daemon.py"])
        processes.append(daemon_proc)
        
        # 4. Start the Live Stream Simulator in the background
        print("▶️  Starting Live Stream Simulator (scripts/stream_simulator.py)")
        stream_proc = subprocess.Popen([sys.executable, "scripts/stream_simulator.py"])
